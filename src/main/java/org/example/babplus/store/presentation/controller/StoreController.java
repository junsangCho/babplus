package org.example.babplus.store.presentation.controller;
...
import org.example.babplus.store.presentation.dto.response.GetStoreResponse;
import org.example.babplus.store.presentation.dto.response.PatchStoreResponse;
import org.example.babplus.store.application.service.StoreService;
...
public class StoreController {
    ...
    public CommonResponse<?> register(@Parameter(hidden = true) @AuthenticationPrincipal UserDetails userDetails, @RequestBody RegisterStoreRequest request){
        var paramVO = RegisterCommand.of(request, userDetails.getUsername());
    ...
    public CommonResponse<?> patch(@PathVariable("store-id") Long storeId, @RequestBody PatchStoreRequest request){
        var paramVO = PatchCommand.of(request, storeId);
